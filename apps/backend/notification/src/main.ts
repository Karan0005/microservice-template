...
async function bootstrap() {
  ...
  const routePrefix: string = configService.get('server.routePrefix') || 'notification/v1';
  ...
  const options = new DocumentBuilder()
    .setTitle('corporate Notification Microservice')
    .setDescription('Api Documentation')
    .setVersion('1.0.0')
    .build();
  const document = SwaggerModule.createDocument(app, options, { include: [AppModule, EmailModule, PubNubModule] });
